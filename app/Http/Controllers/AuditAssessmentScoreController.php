...
use App\Services\AuditAssessmentScoreService;
use Illuminate\Http\Request;

class AuditAssessmentScoreController extends Controller
{
    public function store(Request $request, AuditAssessmentScoreService $auditAssessmentScoreService): \Illuminate\Http\JsonResponse
    {
        $responseData = $auditAssessmentScoreService->store($request);
    ...
    public function list(Request $request, AuditAssessmentScoreService $auditAssessmentScoreService): \Illuminate\Http\JsonResponse
    {
        $responseData = $auditAssessmentScoreService->list($request);
        if (isSuccessResponse($responseData)) {
            $response = responseFormat('success', $responseData['data']);
        } else {
            $response = responseFormat('error', $responseData['data']);
    ...
    public function responseOfRpuMemo(Request $request, AcMemoService $acMemoService): \Illuminate\Http\JsonResponse
    {
        $auditMemoRecommendationStore = $acMemoService->responseOfRpuMemo($request);
