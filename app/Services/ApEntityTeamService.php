...
class ApEntityTeamService
{
    ...
    public function storeAuditTeam(Request $request): array
    {
        ...
        $teams = $teams['teams'];
        try {
            $response = $this->saveAuditTeam($teams, $request);
    ...
    public function saveAuditTeam($teams, Request $request)
    {
        try {
            $parent_id = 0;
            ...
            foreach ($teams['all_teams'] as $team) {
                ...
                $auditVisitCalendarPlanTeam->yearly_plan_location_id = $request->yearly_plan_location_id;
            ...
            return ['status' => 'success', 'data' => 'Successfully Saved Team'];
...
//                    'annual_plan_id' => $request->annual_plan_id,
    ...
    public function storeTeamSchedule(Request $request): array
    {
        ...
        try {
            $saveSchedule = $this->saveTeamSchedule($team_schedules, $yearly_plan_location_id);
    ...
    public function saveTeamSchedule($team_schedules, $yearly_plan_location_id)
    {
        try {
            DB::beginTransaction();
            ...
            foreach ($team_schedules as $designation_id => $schedule_data) {
                ...
                $team_data = AuditVisitCalendarPlanTeam::where('yearly_plan_location_id', $yearly_plan_location_id)
